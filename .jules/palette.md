...
## 2024-10-24 - Semantic Links with Framer Motion

**Learning:** When using Framer Motion with custom button components in React, using `motion.button` with an `onClick` that does `window.location.href` creates severe accessibility issues (screen readers won't announce it as a link, no right-click to open in new tab). Making the component polymorphic to support `motion.a` requires careful TypeScript typing (discriminated unions) and CSS styling (`textDecoration: 'none'`) to prevent visual regressions while enabling semantic navigation. When migrating to hash links (e.g. `#quickstart`), a custom `onClick` handler utilizing `e.preventDefault()`, `scrollIntoView`, and `window.history.pushState` is required to preserve smooth scrolling and avoid jarring default browser jump behavior.
**Action:** Always verify if a 'button' component is being used for navigation. If it is, implement a polymorphic variant that natively renders an `<a>` tag, and explicitly pass an `href` prop.

## 2024-10-24 - Resolving Collateral Damage in `package-lock.json`
**Learning:** During UI updates, accidentally installing packages to verify issues in a server directory without `ignore-scripts` disabled can cause significant `package-lock.json` churn (like adding `sqlite3` from build scripts), breaking CI pipeline tests which strictly rely on exactly locked environments. Also, when modifying a backend file (`index.test.js`) to fix a preexisting failing test due to API implementation mismatch (SSE streaming instead of standard REST), always verify `git status` locally and revert `package.json` and `lock` changes before committing.
**Action:** When working on UI changes, if asked to run backend tests, explicitly isolate environments and clean up un-needed `package.json` updates or use `npx` when possible rather than executing local installs which can mutate the tree.
