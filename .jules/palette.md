...
## 2024-11-20 - Polymorphic UI Buttons
**Learning:** React UI components (like `Button`) that function as navigation links are frequently implemented using `<button>` tags with `onClick` navigation handlers. This breaks semantic HTML, prevents correct screen reader announcements, and disables native browser behaviors like 'Open in New Tab'.
**Action:** When creating or modifying generic UI button components, they must be polymorphic. Conditionally render them as anchor (`<a>`) tags when an `href` prop is provided to preserve native link behaviors and semantic correctness.
