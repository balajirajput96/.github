...
## 2024-03-22 - Polymorphic React UI Components for Semantic Links
**Learning:** When using React components like `Button` for navigation, simply adding an `onClick` that changes `window.location.href` breaks accessibility by ignoring semantic HTML, removing middle-click functionality, and failing to provide context to screen readers.
**Action:** Implement polymorphic components for navigation elements. Conditionally render an `<a>` tag when an `href` prop is present to preserve semantic HTML, native browser behavior, and accessibility while maintaining visual consistency. Use TypeScript discriminated unions to prevent prop collision errors.
