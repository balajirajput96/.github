...
## 2024-05-18 - [Preventing Command Injection in Express Routes]
**Vulnerability:** The application used `child_process.exec` to run local Python scripts directly within the Express request handlers for `/api/jobs/run` and `/api/resume/download`. While no direct user input was currently injected into the command string, using `exec` spawns a shell and is a security anti-pattern, risking command injection if future modifications introduce unescaped user input into the execution string.
**Learning:** Hardcoded system paths or shell scripts are safer to execute directly as binary files rather than through a sub-shell layer, representing a core defense-in-depth practice.
**Prevention:** Always use `child_process.execFile` instead of `exec` when executing specific binaries or scripts. This approach directly executes the file without spawning an intermediate shell, neutralizing command injection risks even if parameters become dynamic later.
