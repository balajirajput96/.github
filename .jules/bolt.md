...
## 2024-05-24 - Pure CSS Typing Animation
**Learning:** High-frequency state updates in React (e.g., `setInterval` for a typing effect) cause expensive continuous re-renders and memory allocations, especially in components with many static elements or lists.
**Action:** Replace `setInterval`-based typing effects with pure CSS `@keyframes` animations (e.g., `clip-path` or `width` with `steps()`) to offload the animation work to the browser's compositor and prevent React render cycles.
