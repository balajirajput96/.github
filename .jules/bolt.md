...
## 2023-11-09 - [React Optimization: Static Arrays in Render]
**Learning:** Generating arrays with random data (like `Math.random()`) directly inline inside a functional component causes the array and all its random values to be re-created on every render cycle. This is problematic for libraries like Framer Motion, which might rely on stable properties to maintain animation state, and it wastes CPU cycles on array allocation.
**Action:** Extract generation of entirely static data arrays that do not depend on props or state completely outside of the component function to ensure the array reference and its contents remain perfectly stable across component updates.
