...
export function TerminalPreview() {
  ...
  useEffect(() => {
    ...
    // Total typing time + small delay before next line
    const currentCmdLength = lines[currentLine].cmd.length;
    const typingTimeMs = currentCmdLength * 100;
          ...
          <div style={{
            ...
            {currentLine < lines.length && (
              <div style={{ display: 'flex', gap: '1rem' }}>
                ...
                >
                  <span className="typing-text">{lines[currentLine].cmd}</span>
                  {TERMINAL_CURSOR}
                </span>
              </div>
            )}
