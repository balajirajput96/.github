...
const githubParamRegex = /^[a-zA-Z0-9_.-]+$/;

app.get('/api/github/repos/:owner', async (req, res) => {
  if (!githubParamRegex.test(req.params.owner)) {
...
app.post('/api/github/issues/:owner/:repo', async (req, res) => {
  if (!githubParamRegex.test(req.params.owner) || !githubParamRegex.test(req.params.repo)) {
