...
describe('Personal AI Platform API', () => {
  ...
  describe('AI Assistant', () => {
    ...
    it('POST /api/assistant/chat uses Gemini when configured', async () => {
      // Create a mock stream that implements the EventEmitter 'on' method
      const mockStream = {
        on: jest.fn((event, callback) => {
          if (event === 'data') {
            const dataString = `data: ${JSON.stringify({ candidates: [{ content: { parts: [{ text: 'Gemini response' }] } }] })}\n\n`;
            callback(Buffer.from(dataString));
          }
          if (event === 'end') {
            callback();
          }
      ...
      // We expect 200 OK since the implementation sends Server-Sent Events chunks
      expect(res.statusCode).toBe(200);
