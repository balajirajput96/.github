...
describe('Personal AI Platform API', () => {
  ...
  describe('AI Assistant', () => {
    ...
    it('POST /api/assistant/chat uses Gemini when configured', async () => {
      ...
      // Simulate the stream emitting SSE formatted data after route listener attaches
      setTimeout(() => {
      ...
      expect(res.statusCode).toBe(200);
      expect(res.text).toContain('data: {"type":"FINAL_RESPONSE","content":"Gemini "}');
      expect(res.text).toContain('data: {"type":"FINAL_RESPONSE","content":"response"}');
