...
describe('Personal AI Platform API', () => {
  ...
  describe('AI Assistant', () => {
    ...
    it('POST /api/assistant/chat uses Gemini when configured', async () => {
      ...
      // Emit data in a timeout so the route handler has time to attach listeners
      setTimeout(() => {
        const payload = { candidates: [{ content: { parts: [{ text: 'Gemini response' }] } }] };
      ...
      expect(res.statusCode).toBe(200);
      expect(res.text).toContain(JSON.stringify({ type: 'FINAL_RESPONSE', content: 'Gemini response' }));
      expect(axios.post).toHaveBeenCalledWith(
