...
describe('Personal AI Platform API', () => {
  ...
  describe('AI Assistant', () => {
    ...
    it('POST /api/assistant/chat uses Gemini when configured', async () => {
      ...
      // Give time for listeners to be attached
      setTimeout(() => {
        stream.emit('data', 'data: {"candidates":[{"content":{"parts":[{"text":"Gemini response"}]}}]}\n\n');
      ...
      expect(res.statusCode).toBe(200);
      // The response from index.js formats it like this:
      // data: {"type":"FINAL_RESPONSE","content":"Gemini response"}
      expect(res.body.toString()).toContain('"content":"Gemini response"');
      expect(axios.post).toHaveBeenCalledWith(
